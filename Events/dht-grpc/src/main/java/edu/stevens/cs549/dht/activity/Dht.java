...
public class Dht extends DhtBase implements IDhtService, IDhtNode, IDhtBackground {
	...
	private NodeInfo getSucc(NodeInfo info) throws Failed {
		...
		if (isEqual(localInfo, info)) {
			...
			return this.getClient().getSucc(info);
		}
	}
	...
	protected OptNodeInfo getPred(NodeInfo info) throws Failed {
		...
		if (isEqual(localInfo, info)) {
			 ...
			 * TODO: Do the Web service call
			 */
			return this.getClient().getPred(info);
		}
	}
	...
	protected NodeInfo closestPrecedingFinger(NodeInfo info, int id) throws Failed {
		...
		if (isEqual(localInfo, info)) {
			...
			if (IRouting.USE_FINGER_TABLE) {
				 ...
				 */
				return this.getClient().closestPrecedingFinger(info, id);
			} else {
				/*
	...
	public OptNodeBindings notify(NodeBindings predDb) {
		...
		if (!pred .hasNodeInfo() || inInterval(cand.getId(), pred.getNodeInfo().getId(), info.getId(), false)) {
			...
			synchronized (state) {
				...
				// Grab all bindings from db
				// I don't know why I can't use List<Bindings> as variables so this is going in a for each loop
				Log.debug(TAG, "notify: Informing any nodes with listeners for transferred bindings");
				for (Bindings binding : db.getBindingsList() )
				{
					this.state.getBroadcaster().broadcastMovedBinding(binding.getKey());
				}
			}
	...
	protected String[] get(NodeInfo n, String k) throws Failed {
		if (isEqual(n, getNodeInfo())) {
			 ...
			 * TODO: Do the Web service call.
			 */
			return this.getClient().getBindings(n, k).getValueList().toArray(new String[0]);
		}
	}
	...
	public void add(NodeInfo n, String k, String v) throws Failed {
		if (isEqual(n, getNodeInfo())) {
			 ...
			 * TODO: Do the Web service call.
			 */
			this.getClient().addBinding(n, k, v);
		}
	}
	...
	public void add(String k, String v) throws Invalid {
		...
		if (pred.hasNodeInfo() && inInterval(kid, pred.getNodeInfo().getId(), info.getId(), true)) {
			 ...
			 */
			this.state.getBroadcaster().broadcastNewBinding(k, v);
	...
	public void delete(NodeInfo n, String k, String v) throws Failed {
		if (isEqual(n, getNodeInfo())) {
			 ...
			 * TODO: Do the Web service call.
			 */
			this.getClient().deleteBinding(n, k, v);
		}
	}
	...
	public void join(String host, int port) throws Failed, Invalid {
		...
		// We assumed that we have join the DHT network.
		// Get the Node Info of the node we want to join
		// IDK if we can reuse $info since we have a new host and port, so we're just
		// going to rebuild the NodeInfo
		NodeInfo.Builder builder = NodeInfo.newBuilder();
		builder.setId(currID);
		builder.setHost(host);
		builder.setPort(port);
		NodeInfo newNodeInfo = builder.build();

		// Find the successor
		//System.out.println("We are about to call successor with " + currID);
		succ = this.client.findSuccessor(newNodeInfo, currID);

		// Then, actually set the successor of this
		this.setSucc(succ);
		//System.out.println("We called successor " + this.getSucc().getId());

		// Apparently we need to call stabilize
		//System.out.println("We are about to call stabilize with " + currID);
		this.stabilize();
	}
...
//			System.out.println("Added listener!");
			this.state.getBroadcaster().addListener(listenerId, key, eventProducer);
//			eventProducer.onNewBinding(key, listenerId + "");
			...
			// Note to self: I think I remember from class that the way to reset bindings
			// is to make a fake call to move binding
			this.state.getBroadcaster().broadcastMovedBinding(key);
		}
	}
	...
	public void listenOff(int listenerId, String key) {
		...
		this.state.getBroadcaster().removeListener(listenerId, key);
	}
	...
	public void stopListening(String key) throws DhtBase.Failed {
		...
		// We need to shut down a subscription. Let's construct one from the key
		// Copying code for listenOn

		// Clientside event - and we need to find the server the key is located on
		//		NodeInfo nodeInfo = this.getNodeInfo();
		//		Subscription subscription = Subscription.newBuilder().setId(nodeInfo.getId()).setKey(key).build();
		//		this.client.listenOff(nodeInfo, subscription);
		//
		//		this.state.stopListening(key);

		// Okay spent like 2 hours debugging this but I think we found the problem
		// We want to FIND THE NODE that has the key we want to get rid of - and then tell it to get rid of that key
		// The first line can stay tho bc we're only trying to get key value
		Subscription subscription = Subscription.newBuilder().setId(this.getNodeInfo().getId()).setKey(key).build();
		NodeInfo nodeToDelete = findSuccessor(NodeKey(key));
		this.client.listenOff(nodeToDelete, subscription);
