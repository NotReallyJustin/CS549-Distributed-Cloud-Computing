...
/**
 * The main class for the client code
 * @author dduggan (Justin Chen for HW)
 */
public class Client {
	...
	public Client(String[] args) {
		try {
			...
			// To Ask: Is this the proxy server, or is this the actual server?
			...
			/*
			 * TODO: Get a server proxy. (In progress)
			 * We have IServerFactory objects that we need to interact with to get the IServer here
			 * So we're just gonna use the client stub from Canvas
			 */

			// It's deprecated for some reason, but it should still be supported
			// System.setSecurityManager(new RMISecurityManager());

			Registry stubRegistry = LocateRegistry.getRegistry(serverAddr, serverPort);
			IServerFactory serverFactory = (IServerFactory) stubRegistry.lookup(serverName);
			...
			public void run() {
				try {
					...
					try {
						...
						log.info("Running GetThread on Client");
						/*
						 * TODO: Complete this thread.
						 *  In active mode, the server will reach out to this client and connect
						 * In a get request, we read from the socket and output to file (in out)
						 */

						InputStream fileInput = new BufferedInputStream(socket.getInputStream());
		...
		private class PutThread implements Runnable {
			...
			public void run() {
				try {
					...
					try {
						log.info("Received connection request from server on client");
						log.info("Running PutThread on client");
						/*
						 * TODO: Complete this thread.
						 *  In a put request, we read from the file and send to the socket
						 * Input - File. Out --> Socket
						 */

						OutputStream sendOut = new BufferedOutputStream(socket.getOutputStream());
						...
						while (bytesRead > 0)
						{
							sendOut.write(buffer, 0, bytesRead);
							bytesRead = this.in.read(buffer, 0, buffer.length);
						}
				...
				try {
					if (mode == Mode.PASSIVE) {
						...
						try {
							 ...
							 *  We grab the input stream from the socket and chuck it in the OutputStream
							 */

							InputStream transferInput = new BufferedInputStream(socket.getInputStream());

							byte[] buffer = new byte[512];
							int numBytes = transferInput.read(buffer, 0, buffer.length);

							while (numBytes > 0)
							{
								out.write(buffer, 0, numBytes);
								numBytes = transferInput.read(buffer, 0, buffer.length);
							}
			...
			if (inputs.length == 2) {
				try {
					if (mode == Mode.PASSIVE) {
						/*
						 * TODO
						 *	Handle reading from our thing and sending it to the server
						 */
						...
						/*
						 * Open the output local file and connect to the server socket.
						 * Remember we need to be prepared to send the file data out there
						 */
						...
						try
						{
							OutputStream transferOutput = new BufferedOutputStream(socket.getOutputStream());

							byte[] buffer = new byte[512];
							int numBytes = in.read(buffer, 0, buffer.length);

							while (numBytes > 0) {
								transferOutput.write(buffer, 0, numBytes);
						...
						/*
						 * TODO
						 *  In active mode, the server connects to us. We need to accept.
						 * Hence, launch a put thread
						 */
