...
public class Dht extends DhtBase implements IDhtService, IDhtNode, IDhtBackground {
	...
	private NodeInfo getSucc(NodeInfo info) throws Failed {
		...
		if (isEqual(localInfo, info)) {
			...
			// We're calling the WebClient to send succ to the right node
	...
	protected String[] get(NodeInfo n, String k) throws Failed {
		if (isEqual(n, getNodeInfo())) {
			...
			// Really ugly, but this is the gist:
			// Get the binding from the web request call
			// Then, all the values associated with the key
			// Convert it to an array w/ array to store stuff in (intellij automatically finished this code)
