...
public class State implements IState, IRouting {
	...
	public synchronized NodeInfo closestPrecedingFinger(int id) {
		...
		// We took the code for the closest preceding finger from the PPT slides
		// finger[i] E (n, id)	--> finger[i] is within the current node's ID and the wanted ID

		int currNodeID = this.getNodeInfo().getId();

		// Dunno if NFINGERS is the right one here, but I saw it being used in routes()
		for (int i = NFINGERS - 1; i >= 0; i--)
		{
			if (DhtBase.inInterval(this.getFinger(i).getId(), currNodeID, id, false))
			{
